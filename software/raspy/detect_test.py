...
class DetectTest:
    ...
    def main(self):
        ...
        while True:
            ...
            src_d = cv2.resize(src, self.win_size, interpolation=cv2.INTER_CUBIC)
            cv2.imshow('src', src_d)
            mask_d = cv2.resize(mask, self.win_size, interpolation=cv2.INTER_CUBIC)
            cv2.imshow('hsv_mask', mask_d)
            cv2.setMouseCallback('hsv', self.mouse_callback, hsv)
            hsv_d = cv2.resize(hsv, self.win_size, interpolation=cv2.INTER_CUBIC)
